...
pip download -d save_dir -r requirements.txt #根据requirements下载轮子（它会顺便下载这些库依赖的库，所以requirements可以只写你需要的核心库）
pip install --no-index --find-links=save_dir -r requirements.txt #在离线电脑上安装
